  ...
  return (
    <main className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center py-8 sm:py-12 px-4 relative bg-gradient-to-br from-brand-black via-brand-dark to-brand-black">
        {/* Decorative elements */}
        <div className="hidden sm:block absolute top-0 right-0 w-48 md:w-96 h-48 md:h-96 bg-brand-red/5 rounded-full blur-3xl" />
        <div className="hidden sm:block absolute bottom-0 left-0 w-48 md:w-96 h-48 md:h-96 bg-brand-red/5 rounded-full blur-3xl" />
        ...
        <div className="bg-brand-surface/50 backdrop-blur-sm border border-white/5 rounded-sm p-5 sm:p-8">
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            {/* Error Message */}
            {error && (
              <div className="bg-red-500/10 border border-red-500/30 rounded-sm p-3 text-red-400 text-sm text-center">
                {error}
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full btn-primary py-3 sm:py-4 text-sm sm:text-base ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              {isLoading ? (
          ...
          <div className="relative my-6 sm:my-8">
            ...
            <div className="relative flex justify-center text-xs sm:text-sm">
              <span className="px-4 bg-brand-surface/50 text-gray-500">or continue with</span>
            </div>
          </div>
      ...
      {/* Sign Up Link - Black background section */}
      <div className="bg-brand-black py-6 sm:py-8 px-4">
        <p className="text-center text-sm sm:text-base text-gray-500">
          Don't have an account?{' '}
          <Link to="/signup" className="text-brand-red hover:text-brand-red-light transition-colors font-medium">
            Sign up
          </Link>
        </p>
      </div>
